...
const Monitor: React.FC = () => {
  ...
  useEffect(() => {
    if (!loadingControl && questions.length > 0 && (!session || session.status === 'finished')) {
      initializeSession();
    }
  ...
  if (loadingControl || !session) return <div className="p-12 text-center">Loading Control Room...</div>;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-zinc-50">
      <div className="flex-1 flex flex-col md:flex-row h-[calc(100vh-80px)] overflow-hidden">
        ...
        <div className="flex-1 p-6 overflow-y-auto border-r border-zinc-200">
          <BlurReveal className="max-w-3xl mx-auto space-y-6">
            <div className="flex justify-between items-end border-b border-zinc-200 pb-4">
              ...
              <div className="text-right">
                <div className="text-3xl font-mono font-bold">{responsesCount}</div>
                <div className="text-xs uppercase tracking-widest text-zinc-400">Responses</div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-zinc-200 p-8 flex flex-col items-center text-center space-y-6">
               ...
               {currentQ?.imageUrl && (
                 <img src={currentQ.imageUrl} className="h-40 object-contain rounded border border-zinc-100" alt="Q" />
               )}
               ...
               {(session.status === 'active' || session.status === 'revealed') && (
                 <div className="grid grid-cols-2 gap-4 w-full max-w-lg text-left">
                   {currentQ?.options.map((opt, i) => (
                     <div key={i} className={`p-3 rounded border text-sm ${i === currentQ.correctIndex ? 'border-green-500 bg-green-50 font-bold' : 'border-zinc-100 bg-zinc-50 text-zinc-400'}`}>
                       {opt}
                     </div>
                   ))}
                 </div>
        ...
        <div className="max-w-6xl mx-auto w-full flex items-center justify-between gap-4">
          ...
          <div className="flex gap-4">
            {session.status === 'waiting' && (
               <Button onClick={() => revealOptions(session.currentQuestionIndex)} className="w-40 bg-black">
                 Start Timer
               </Button>
            )}

            {session.status === 'active' && (
               <Button onClick={() => revealAnswer(session.currentQuestionIndex)} className="w-40 bg-zinc-800">
                 Reveal Answer
               </Button>
            )}
            ...
            {session.status === 'revealed' && (
              isLastQuestion ? (
                <Button onClick={() => { endQuiz(); navigate('/admin/dashboard'); }} variant="destructive">
                  End Quiz
                </Button>
              ) : (
                <Button onClick={() => pushQuestion(session.currentQuestionIndex + 1)} className="w-40">
                  Next Question
                </Button>
              )
            )}
            ...
            {session.status === 'finished' && (
               <Button onClick={() => navigate('/admin/dashboard')} variant="outline">Back to Dash</Button>
            )}
          </div>
        </div>
      </div>
