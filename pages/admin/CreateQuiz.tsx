...
const CreateQuiz: React.FC = () => {
  ...
  const updateQuestion = (idx: number, field: keyof LocalQuestion, value: any) => {
    ...
    <div className="max-w-3xl mx-auto p-6 pb-24 space-y-8">
      <BlurReveal>
        ...
        {!hasCloudinary && (
          <p className="text-xs text-yellow-600 mt-2">
            Image upload disabled. Configure Cloudinary keys in Login screen to enable.
          </p>
        )}
      </BlurReveal>

      <BlurReveal delay={0.1} className="space-y-4">
        <label className="block text-sm font-medium text-zinc-500">Quiz Title</label>
        <input 
      ...
      <div className="space-y-12">
        {questions.map((q, qIdx) => (
          <BlurReveal key={qIdx} delay={0.1 + (qIdx * 0.05)} className="p-6 border border-zinc-200 rounded-xl space-y-4 shadow-sm">
            <div className="flex justify-between items-center">
               <span className="text-xs font-mono bg-zinc-100 px-2 py-1 rounded">Q{qIdx + 1}</span>
               {hasCloudinary && (
                 <div className="relative">
                   <input 
                     type="file" 
                     className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                     onChange={(e) => e.target.files?.[0] && handleImageUpload(e.target.files[0], qIdx)}
                     disabled={!!uploadingImg}
                   />
                   <Button size="sm" variant="outline" type="button">
                     {uploadingImg === qIdx ? 'Uploading...' : q.imageUrl ? 'Change Image' : 'Add Image'}
                   </Button>
                 </div>
               )}
            </div>
            ...
            {q.imageUrl && (
              <img src={q.imageUrl} alt="Question" className="h-32 object-contain rounded-lg border border-zinc-100 bg-zinc-50" />
            )}
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white/80 backdrop-blur-md border-t border-zinc-200 p-4 flex justify-center gap-4 z-10">
        ...
        <Button onClick={handleSubmit} isLoading={submitting}>Save Protocol</Button>
      </div>
    </div>
  );
};
